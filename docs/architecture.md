...
- ~~**Are Agent Registry, Memory Bank, Agent Identity, Agent Gateway and Agent Observability enabled and reachable** in our project and region?~~ **Agent Registry answered, 27 Aug.** `aiplatform.googleapis.com` is enabled on `warrent-505918` (it now lists as *Agent Platform API*). `AgentService` is **not served from any regional endpoint** — `us-central1`, `us-east4`, `us-west1` and `europe-west4` all return `INVALID_ARGUMENT: AgentService not supported in this location` — and **is** served from `global`, where `GET .../locations/global/agents` returns `{}`. So it is reachable and empty. It stays empty because `Agent.base_agent` is required, immutable, and accepts only `antigravity-preview-05-2026`; the resource models Antigravity agents, and ours are `google-genai` agents on Agent Engine. Registering them would publish a roster that is not the fleet serving traffic. Model Armor remains confirmed; Memory Bank, Identity, Gateway and Observability are still unprobed.
- ~~**Can procedures be modelled in Agent Registry at all?**~~ **Resolved by taking the fallback.** Procedures live in Firestore with immutable versions at `/tenants/{t}/procedure_versions/{id}:{n}`, and a job pins the version it started under, so publishing v3 cannot change what a running v2 job is executing. The verifier agents are held by Agent Engine and enumerated by the fleet's own `roster()` — not by the Registry, for the reason above.
